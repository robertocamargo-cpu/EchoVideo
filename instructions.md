...
1. **Dynamic Zoom-In Drift**
   ...
   - **Instruction for Render**: Linearly increase the image scale from 1.15 to 1.25 while shifting the horizontal axis based on the direction tag: for move:right shift from -2% to +2% or for move:left shift from +2% to -2%, keeping the vertical axis centered throughout the scene duration.
...
2. **Contextual Zoom-Out Reveal**
   ...
   - **Instruction for Render**: Linearly decrease the image scale from 1.25 to 1.15 while shifting the horizontal axis based on the direction tag: for move:right shift from -2% to +2% or for move:left shift from +2% to -2%, ensuring the movement remains subtle and the vertical axis stays fixed at the center.
...
### REGRA DE OURO — SEM ESTILO:
NÃO inclua palavras sobre estilo artístico, iluminação, câmera ou qualidade (nada de "4k", "cinematic", "cartoon"). Descreva APENAS a matéria física (pele, tecido, cabelo).
...
Se o nome for de uma figura histórica ou celebridade real:
- Descrever os **traços faciais específicos** (formato do nariz, olhos, estrutura óssea)
- **NUNCA citar o nome real** nos prompts de criação de imagem
...
A IA deve evitar criar personagens genéricos. Cada personagem deve ter:
...
### Variáveis Obrigatórias (TODAS devem ser respondidas — nenhuma pode ficar indefinida):
- `[TIPO DO OBJETO]`
- `[MATERIAL PRINCIPAL]`
- `[COR PRINCIPAL]`
- `[TEXTURA/SUPERFÍCIE]`
...
O prompt enviado para a IA de imagem e exportado no arquivo `.txt` deve seguir rigorosamente esta ordem de concatenação:

`[NOME_DO_ESTILO]: [MEDIUM], [SUBJECT], [QUANTITY], [ACTION], [CENARIO], [PROPS_PROMPT]. style: [STYLE_PROMPT], [CAMERA]`

### Regras de Concatenação:
1. **[NOME_DO_ESTILO]**: O nome amigável do estilo (ex: "Anime Retro").
2. **[SUBJECT]**: Se houver mais de um personagem na cena, concatene as descrições físicas de todos usando " AND ".
3. **[QUANTITY]**: Se houver mais de um personagem, inclua explicitamente a contagem (ex: "Quantity: 2 characters").
4. **[PROPS_PROMPT]**: Se houver objetos vinculados, use o prefixo "Featuring object(s): " seguido das descrições.
5. **[STYLE_PROMPT]**: O prompt técnico configurado na galeria de estilos.
6. **PROIBIDO**: Nunca inclua nomes reais de pessoas ou palavras de qualidade como "4k", "HD", "realistic" no prompt final.
